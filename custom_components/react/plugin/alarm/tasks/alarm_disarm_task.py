...
from custom_components.react.const import REACT_ACTION_DISARM, REACT_TYPE_ALARM
...
class AlarmDisarmTask(PluginReactionTask):
    ...
    async def async_execute_plugin(self, event: AlarmDisarmReactionEvent):
        ...
        await self.api.async_alarm_disarm(event.context, event.payload.entity)
...
class AlarmDisarmReactionEvent(ReactionEvent[AlarmDisarmReactionEventData]):
    ...
    def applies(self) -> bool:
        return (
            ...
            self.payload.action == REACT_ACTION_DISARM and 
