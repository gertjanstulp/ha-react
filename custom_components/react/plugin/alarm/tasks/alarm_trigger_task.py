...
from custom_components.react.const import REACT_ACTION_TRIGGER, REACT_TYPE_ALARM
...
class AlarmTriggerTask(PluginReactionTask):
    ...
    async def async_execute_plugin(self, event: AlarmTriggerReactionEvent):
        ...
        await self.api.async_alarm_trigger(event.context, event.payload.entity)
...
class AlarmTriggerReactionEvent(ReactionEvent[AlarmTriggerReactionEventData]):
    ...
    def applies(self) -> bool:
        return (
            ...
            self.payload.action == REACT_ACTION_TRIGGER and 
