...
class PersonStateChangeInputBlock(StateChangeInputBlock[DynamicData]):
    def __init__(self, react: ReactBase) -> None:
        super().__init__(react, DOMAIN)


    def read_state_data(self, react_event: StateChangedEvent) -> StateChangeData:
        return BinaryStateChangeData(react_event.payload, on_state=STATE_HOME, off_state=STATE_NOT_HOME)
